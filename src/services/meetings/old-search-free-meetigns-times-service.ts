...
interface OldGetDayFreeMeetingTimesServiceRequest {
  squad_id: string;
  date: Date;
}
...
export class OldGetDayFreeMeetingTimesService {
  constructor(
  ...
  async execute({
    squad_id,
    date,
  }: OldGetDayFreeMeetingTimesServiceRequest) {
    ...
    // Agora o work_time é um array de work_times
